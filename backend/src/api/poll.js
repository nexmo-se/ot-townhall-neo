...
class PollAPI{
  ...
  static async list({ sessionID }: IList): Promise<Poll[]>{
    return await DatabaseAPI.query(async (client: any) => {
  ...
  static async poll({ pollID, itemID, userID, name }: IPoll){
    await DatabaseAPI.query(async (client: any) => {
      await client.query(
  ...
  static async update({ pollingID, status }: IUpdate){
    await DatabaseAPI.query(async (client: any) => {
