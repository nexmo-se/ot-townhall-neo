...
function QuestionDownload () {
  ...
  /**
   * Retrieve the questions from the API endpoint.
   * It will retrieve all questions for the session.
   */
  const retrieveQuestions = React.useCallback(
    async () => {
      try {
        const sessionId = lodash(session).get("sessionId");
        const response = await fetch(`${config.apiURL}/questions?session_id=${sessionId}`);
        const questions = await response.json();
        
        return questions.map(q => ({
          owner_name: q.owner?.name || q.owner_name,
          content: q.content,
          vote: q.vote || 0,
          status: q.status,
          created_at: DateTime.fromISO(q.created_at)
        }));
      } catch (e) {
